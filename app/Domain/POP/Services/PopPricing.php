 ...
 public function quote(array $input): array
{
    ...
    $spec = $specs->first(function($s) use ($input) {
        return $s->material   === $input['material']
            && $s->side       === $input['side']
    ...
    // ❌ HAPUS validasi kelipatan min_qty/step_qty
    // if ($qty < $spec->min_qty || ($qty - $spec->min_qty) % max(1, $spec->step_qty) !== 0) { ... }

    // ✅ Pilih tier grosir berdasar breakpoint min_qty
    //    - Cari tier dengan min_qty <= qty (terbesar)
    //    - Jika tidak ada (qty < tier terkecil), pakai tier terkecil sebagai harga eceran
    $tiers = $spec->priceTiers->sortBy('min_qty')->values();
    $tier = $tiers->filter(fn($t) => $t->min_qty <= $qty)->sortByDesc('min_qty')->first()
         ?? $tiers->first(); // fallback ke tier paling kecil

    if (!$tier) {
        throw ValidationException::withMessages(['pricing' => 'Tier harga belum diset untuk spesifikasi ini.']);
    }

    $unit = (float) $tier->unit_price;

    // Cutting fee (contoh, tetap per 100 lembar)
