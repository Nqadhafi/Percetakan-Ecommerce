...
const props = defineProps({
  stats: { type: Object, default: () => ({ pending: 0, in_production: 0, completed: 0 }) },
...
const profilePercent = computed(() => {
  ...
  if (props.profile?.address_json) score += 30
...
const badgeClass = s => ({
  ...
  in_production: 'bg-sky-50 border-sky-200 text-sky-700',
  ...
  <div class="space-y-6">
      ...
      <div class="lg:col-span-2 border rounded-xl bg-white p-4">
        ...
        <div class="mt-4">
          ...
          <ul class="mt-3 text-sm text-gray-600 space-y-1">
            ...
            <li>• Alamat: <b>{{ props.profile?.address_json?.street + ',' + props.profile?.address_json?.city  ?? '—' }}</b></li>
          </ul>
        </div>
      </div>
    ...
    <div class="border rounded-xl bg-white">
      <div class="flex items-center justify-between p-4">
        ...
        <Link href="/orders" class="text-sm text-blue-600 hover:underline">Semua Pesanan</Link>
      </div>
        ...
        <table class="w-full text-sm border-t">
          ...
          <tbody>
            <tr v-for="o in recentOrders" :key="o.id" class="border-t">
              ...
              <td class="px-4 py-2">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs border" :class="badgeClass(o.status)">
                  {{ o.status }}
                </span>
              </td>
              <td class="px-4 py-2 text-right">Rp {{ fmtMoney(o.total) }}</td>
              <td class="px-4 py-2 text-right">
                <Link :href="o.show_url" class="text-blue-600 hover:underline">Lihat</Link>
              </td>
            </tr>
          </tbody>
        </table>
